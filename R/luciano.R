...
require(maps)
require (scales)
query <- pbdb_occurrences (limit="all", vocab= "pbdb", base_name="Canis", show="coords")
...
pbdb_plot<-function(query,col.int='white',  col.ocean='black',fill=T,main=NULL, 
             col.point=c('light blue','blue'),...){
    par(mar=c(0,0,0,0),oma=c(6, 0, 0, 0),xpd=TRUE)
    map(t='n',...)
    .add.ColOcean(col.ocean)
    map(col=col.int,fill=fill,add=T,...)
    data <- .extract.LatLong(query)
    dat<-.add.Points(data,col.point,...)
    title(main=main,line=1,...)
    
    .add.Legend(dat,col.int,...)
    #     legend("bottom",col=unique(dat$Col)[order(unique(dat$n))], inset=c(0,-0.11), 
    #            legend=range(), pch=16,ncol=n, title="Occurrences")
}

#x11()
system.time(pbdb_plot(query,pch=19,col.point=c('light blue','blue')))
